...
namespace K {
    ...
    // Düsen-Verteilung nach Radius (KORRIGIERT für exakt 61.440)
    // Berechnet aus: 441 Zellen × gewichtete Verteilung
    constexpr int NOZZLES_KERN = 262;             // Radius 0-3 (29 Zellen)
    constexpr int NOZZLES_MITTEL = 175;           // Radius 4-7 (120 Zellen)
    constexpr int NOZZLES_RAND = 131;             // Radius 8-10 (168 Zellen)
    constexpr int NOZZLES_AUSSEN = 87;            // Radius >10 (124 Zellen)
    ...
    // G0 = 8/9 - IMMER als Bruch oder mit 17 Dezimalstellen!
    constexpr double FALZ_FACTOR = 0.88888888888888889;  // 8/9 EXAKT
}
...
struct AnkerCell {
    ...
    void init(int px, int py) {
        ...
        // Düsen-Anzahl nach Radius (KORRIGIERT für exakt 61.440)
        int nozzle_count;
        if (radius <= K::RADIUS_KERN) {
            nozzle_count = K::NOZZLES_KERN;       // 262
        } else if (radius <= K::RADIUS_MITTEL) {
            nozzle_count = K::NOZZLES_MITTEL;     // 175
        } else if (radius <= K::RADIUS_RAND) {
            nozzle_count = K::NOZZLES_RAND;       // 131
        } else {
            nozzle_count = K::NOZZLES_AUSSEN;     // 87
        }
...
struct PurityGuard {
    static constexpr double RED_LINE = 10.0 / 81.0;  // ≈ 0.1235
